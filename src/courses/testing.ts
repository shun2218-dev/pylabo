...
`,
            hint: [
              "何を書くか迷ったら、仕様表の 1 行をテスト 1 つに割り当てるところから始めます。「税込になること」「負の価格で止まること」「0 人で止まること」「正しく割れること」で、もう 4 件です。",
              "`add_tax(100)` は 110 になるはずですが、小数の掛け算なので `==` では落ちます。「だいたい等しい」を扱う道具が本文にあり、期待値のほうを包む形で使います。",
              "例外のテストは「呼んだら止まる」ことを確かめるものなので、`assert` では書けません。「このブロックの中で例外が上がるはず」と宣言する書き方を使い、その中でわざと不正な引数を渡します。",
              "採点は、あなたのテストを「例外チェックを外した実装」と「税込計算を壊した実装」にも走らせます。前者で落ちるには例外のテストが 2 つの関数それぞれに要り、後者で落ちるには金額そのものを確かめるテストが要ります。",
            ],
...
`,
            hint: [
              "この関数がテストしにくいのは、「今が何時か」と「実際に送ること」に依存しているからです。放っておくと結果が日によって変わるので、どちらもテスト側から固定できるものに置き換えるのが出発点です。",
              "差し替える対象は、`reminder` モジュールが持っている名前そのものです（`reminder.now` と `reminder.send`）。関数の中から呼ばれるときに見にいく先を書き換える、という発想です。",
              "固定する時刻の選び方が、そのままテストケースの設計になります。全タスクの期限より後ろに置けば全件が期限切れ、手前に置けば 0 件、あいだに置けば一部だけ。3 つのテストをこの 3 通りに対応させると、仕様を漏れなく押さえられます。",
              "確かめるのは戻り値の件数だけでは足りません。採点は「期限を見ずに全件通知する版」でも落ちることを求めます。差し替えた送信側が何回・どんな内容で呼ばれたか、そして 1 件も期限切れが無いときは一度も呼ばれないことまで見てください。",
            ],
...
`,
            hint: [
              "元の関数がテストしにくいのは、「ファイルを開く」「数える」「表示する」の 3 つが 1 つに詰まっているからです。テストしたいのは真ん中だけなので、そこを「受け取って返すだけ」の関数として外に出します。",
              "`summarize` が受け取るのはファイル名でもファイルそのものでもなく、「行の並び」です。テストからは文字列のリストを渡せます。開いたファイルも 1 行ずつ取り出せる並びなので、`print_summary` 側は読み込んだものをそのまま渡せます。",
              "`summarize` の中で `print` してはいけません（採点で確かめています）。値を返すところまでが `summarize`、表示は呼び出した側の仕事、という切り分けがこのレッスンの狙いです。",
              "テストは 2 件以上必要ですが、件数をそろえただけでは通りません。採点はあなたのテストを「空行も数えてしまう版」に対しても走らせます。空行が混ざったケースを 1 件は入れてください。",
            ],
...
`,
            hint: [
              "実装より先にテストです。仕様表の 5 行が、そのまま 5 つのテストケースになります。1 つ書いて `run_pytest()` で赤を見てから実装を足す、を繰り返してください。",
              "考え方は「全員に配る基本額」と「配りきれなかった余り」に分けることです。基本額は割り算の商、余りはその剰余で、この 2 つは同時に求めることもできます。",
              "余りは先頭から 1 円ずつ配ります。言い換えると「先頭から数えて余りの人数ぶんの人だけ、基本額に 1 円足す」。この形なら、合計は必ず元の金額に戻ります。",
              "人数のチェックは割り算より**前**に置いてください。あとに置くと 0 人のときにゼロ除算が先に起きて、`ValueError` にはなりません。採点はあなたのテストを「余りを切り捨てる版」にも走らせるので、割り切れないケースのテストは必ず入れましょう。",
            ],
            solution: `import pytest
...
`,
            hint: [
              "順番が肝心です。まず報告の 2 件をテストとして書き、いま貼られている実装のままで赤くなることを確かめます。赤を見てから直す、というのがこのレッスンの主題です。",
              "報告①は「引きすぎて負になる」。仕様は 0 で止まることなので、引き算の結果をそのまま返すのではなく、下限を決める形になります。",
              "報告②は「負のポイントで支払額が増える」。これは計算する前に弾く話です。チェックを引き算より前に置いてください。",
              "採点はあなたのテストを直す前の実装にも走らせ、2 件以上落ちることを求めます。①と②を別々のテストに分けておきましょう。あわせて普通のケース（使えたぶんだけ引かれる、0 ポイントなら定価のまま）も足して、全体を緑にします。",
            ],
            solution: `import pytest
